...
function renderLineChart(selector, data, config) {
  ...
  const { svg, tooltip, width: W, height: H } = appendTrendFigure(container, title);
    ...
    .on('mousemove', (event) => {
      ...
      series.forEach((s) => {
        ...
        focusDots
          ...
          .attr('class', 'marker-ring')
          .attr('cx', dx)
          .attr('cy', yScale(d[s.key]))
          .attr('r', 4)
          .attr('fill', s.color);
      });

      tooltip.html(
        `<div class="tt-h">${xAccessor(d)}</div>${series.map((s) => `<div class="tt-row"><span class="lab"><span class="k" style="background:${s.color}"></span>${s.label}</span><span class="val">${Number.isFinite(d[s.key]) ? yTickFormat(d[s.key]) : formatCount(d[s.key])}</span></div>`).join('')}`,
      );
